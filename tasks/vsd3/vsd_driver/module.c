...
static const size_t IO_BUF_SIZE = 2 * PAGE_SIZE;
    ...
    char __user *read_user_buf, size_t read_size, loff_t *fpos)
{
     ...
     * 1. After interrupt comes, then starts vsd_dev_dma_op_complete_tsk_func
     * 2. This function insert driver to scheduler queue
     * 3. When driver getting control, it make kernel buffer free
     * 4. When device getting control, it write to buffer
     */
...
static long vsd_ioctl_set_size(vsd_ioctl_set_size_arg_t __user *uarg)
{
    ...
    if (copy_from_user(&arg, uarg, sizeof(vsd_ioctl_get_size_arg_t))){
        return -EFAULT;
    }
    ...
    ret = (long)vsd_dev->hwregs->result;
